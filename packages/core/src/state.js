...
import { generateUUID, generateFilename } from './utils.js';
import { DataService } from '@arc/data';

export const dataDB = new DataService('render');
...
const state = {
    ...
    async init() {
        ...
        if (!dataDB.isSupported() || skipFS) {
            ...
            const restored = await dataDB.restore();
            if (restored) {
                this.storageType = 'fs';
    ...
    async _loadProjectsFromFS() {
        const workspacesArr = await dataDB.listWorkspaces();
        const projects = {};
        for (const w of workspacesArr) {
            projects[w.id] = w;
        }
        ...
        for (const project of Object.values(projects)) {
            // Ensure versioning on every loaded project
            this._ensureVersioning(project);
            for (const img of project.images) {
                loaders.push(
                    dataDB.getImageBase64(img.id).then(b64 => {
                        img.base64 = b64;
                    })
                );
            }
    ...
    async addImage(img, prompt, mode, batchId, parentImageId = null) {
        ...
        // ── Version assignment ──
        if (project.nextVersion == null) project.nextVersion = project.images.length + 1;

        let version, versionSuffix = '';
        const isResolutionIncrease = mode === 'Enhance' && /Resolution \d+x/i.test(prompt);

        if (isResolutionIncrease && parentImageId) {
            // Resolution increases reuse source version with suffix
            const parent = this.getImage(parentImageId);
            version = parent?.version ?? project.nextVersion;
            const scaleMatch = prompt.match(/(\d+)x/i);
            versionSuffix = scaleMatch ? `_${scaleMatch[1]}x` : '_2x';
        } else {
            // Check if this batchId already has a version assigned (batch sharing)
            const existingBatchImage = project.images.find(i => i.batchId === batchId && i.version != null);
            if (existingBatchImage) {
                version = existingBatchImage.version;
            } else {
                version = project.nextVersion;
                project.nextVersion++;
            }
        }
        ...
        imageRecord.filename = generateFilename(imageRecord, project.name, project) + '.png';
    ...
    /**
     * Ensure a project has versioning fields.
     * For existing projects without nextVersion, retroactively assign
     * versions grouped by batchId in chronological order.
     */
    _ensureVersioning(project) {
        if (project.nextVersion != null) return;
        const batches = new Map();
        const sorted = [...project.images].sort((a, b) => a.createdAt - b.createdAt);
        let version = 1;
        for (const img of sorted) {
            if (img.version != null) continue; // already versioned
            if (!batches.has(img.batchId)) {
                batches.set(img.batchId, version);
                version++;
            }
            img.version = batches.get(img.batchId);
            img.versionSuffix = img.versionSuffix ?? '';
            img.filename = img.filename ?? '';
        }
        project.nextVersion = version;
    },
    ...
    async save() {
        ...
        if (this._useFS()) {
            ...
            await dataDB.saveWorkspace(project);
        } else {
