...
export const fsStorage = {
     ...
     * plus a matching sidecar JSON with metadata.
     */
    async saveImage(project, imageId, base64) {
        ...
        if (img) {
            const sidecar = {
                ...
                version: img.version,
                prompt: img.prompt,
                structured_prompt: img.structured_prompt,
                mode: img.mode,
                createdAt: img.createdAt
            };
            try {
                const jsonFh = await dir.getFileHandle(`${imageId}.json`, { create: true });
                const jsonW = await jsonFh.createWritable();
                await jsonW.write(JSON.stringify(sidecar, null, 2));
                await jsonW.close();
            } catch (e) {
                console.warn('[FSStorage] Failed to write sidecar JSON:', e);
            }
        }
    },
